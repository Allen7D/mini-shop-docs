...
- **分层架构**：controller → service → dao → model 的分层思想，是几乎所有主流后端（Java Spring、Node NestJS）的通用骨架。

## 如果你想把这个项目变成自己的作品

- 把数据库（`zerd_pg.sql`）导入本地 MySQL，先让项目真的跑起来。
- 加一个你自己感兴趣的模块（比如「收藏夹」「我的优惠券」），完整走一遍新增接口流程。
- 试着给某个接口加 `@api.limiter` 限流（`libs/limiter.py`），体会「扩展都在壳上」的设计。

::: tip 学习心态
前端开发者学这个后端项目，最容易卡在「为什么分层这么多层」。记住——**分层不是为了复杂，而是为了「改一处不影响其它」**。抓住「请求每层做什么」这条主线，它就是一个可驾驭的真实工程。
:::
