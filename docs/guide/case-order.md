...
这是全项目最有含金量的地方。`palce` 的完整流程是**「先校验库存，通过才生成订单」**：
...
真实的 `palce` **只校验库存，并不扣减库存**。它把订单和商品快照存了下来，真正的库存扣减发生在**支付成功之后**（见 `service/pay.py`）。很多新手会误以为下单时库存就已扣减——这里明确一下：下单阶段只「检查库存 + 记录订单」，不动库存数据。
:::
